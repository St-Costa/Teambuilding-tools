...
// Posizioni (riga, colonna) in una griglia 3×3 per disporre 5 voti come il "5"
// dei dadi: due in alto, uno al centro, due in basso.
const POS_DADO: ReadonlyArray<readonly [number, number]> = [
  [1, 1],
  [1, 3],
  [2, 2],
  [3, 1],
...
// Griglia dei votanti: layout normale a 2 colonne, ma con 5 voti diventa il
// "5" dei dadi (2×2 + 1 al centro).
function GrigliaVotanti({
  ...
  if (votanti.length === 5) {
    return (
      <div
        className={styles.votantiDado}
        style={{
          gridTemplateColumns: `repeat(3, ${dim}px)`,
          gridTemplateRows: `repeat(3, ${dim}px)`,
        }}
      >
        {votanti.map((v, i) => {
          const [riga, col] = POS_DADO[i];
          return (
            <div key={v.personaggioId} style={{ gridColumn: col, gridRow: riga }}>
              <Cerchietto
...
export default function ScenaVoti({ snapshot, folderPath, sfondoSrc }: Props) {
  ...
  // Limite h*0.26 (ridotto rispetto a 0.30) lascia spazio alla riga NPC in basso.
  const dimTarget = Math.min(Math.floor(colWidth * 0.87), Math.floor(h * 0.26));
  // Cerchietti votanti
  ...
  // Riga NPC: cerchietto un po' più piccolo del target così la barra resta bassa.
  const dimNpc = Math.min(dimTarget, Math.floor(h * 0.20));
